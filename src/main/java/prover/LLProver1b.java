...
public class LLProver1b extends LLProver {
     ...
     * @param settings
     */
    public LLProver1b(Settings settings) {
        setSettings(settings);
        this.proofBuilder = new StringBuilder();
    }

    public LLProver1b(Settings settings, StringBuilder proofBuilder) {
    ...
    public void deduce(Sequent seq) throws ProverException, VariableBindingException {
         ...
         //           CGNode childNode = (CGNode) scc2.getEdgeTarget((CGNode) childNodeGraph);
                ...
                if (sccKey.vertexSet().size() == 2) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (node.nodeType.equals(CGNode.type.CATEGORY) && !node.category.contains("-o")) {
                            ...
                            if (categoryGraph2.incomingEdgesOf(node).size() <= 2) {
                                atomicCategory = node;
                                iter.remove();
                                break;
                            }
                    ...
                    if (atomicCategory != null) {
                        ...
                        if (node.nodeType == CGNode.type.CONNECTOR) {
                            List<DefaultEdge> incomingEdges = new ArrayList<>(categoryGraph2.incomingEdgesOf(node));
                            Iterator<DefaultEdge> outIter = incomingEdges.iterator();
                            while (outIter.hasNext()) {
                                DefaultEdge e = outIter.next();
                                if (categoryGraph2.getEdgeSource(e).equals(atomicCategory)) {
                                    outIter.remove();
                                } else {
                                    //Check if second parent of connector node is a modifier of the atomic category
                                    if (categoryGraph2.getEdgeSource(e).category.equals(atomicCategory + " -o " + atomicCategory))
                                        ;
                                    {
                                        basicScc = true;
                                        modifierNode = categoryGraph2.getEdgeSource(e);
                                        outIter.remove();
                                    }
                                }
                            }
                ...
                for (CGNode node : sccKey.vertexSet()) {
                    ...
                    if (node.nodeType.equals(CGNode.type.CATEGORY)) {
                        ...
                        for (Object edge : categoryGraph2.incomingEdgesOf(node)) {
                            CGNode parentConnectorNode = (CGNode) categoryGraph2.getEdgeSource((DefaultEdge) edge);
                            node.histories.addAll(parentConnectorNode.histories);
                            node.compressHistories();
                        }
                    }
                ...
                //Add output histories to their respective output nodes
             ...
             //     outputNode.compressHistories();
                ...
                histories = nonScopingChartDeduce(nonscopingAgenda);
                ...
                for (CGNode outputNode : outputNodes)
                {
                   ...
                   outputNode.compressHistories();
                    ...
                    sortedHistories.sort(new Comparator<History>() {
                        @Override
                        public int compare(History o1, History o2) {
                            return Integer.valueOf(o1.indexSet.size()).compareTo(o2.indexSet.size());
                        }
                    });
        ...
        for (CGNode n : categoryGraph2.vertexSet())
        {
...
*/
    ...
    /**
     * Non-scoping chart deduce applies with no-scope flags. In this case, modifiers are only applied once based on the main index.
     * e.g, a[0], a -o a[1], a -o a[2] will only be combined once.
     * @param histories
     * @return
     * @throws VariableBindingException
     * @throws ProverException
     */
    public List<History> nonScopingChartDeduce(List<History> histories) throws VariableBindingException, ProverException {
        getLOGGER().finer("Beginning a partial chart derivation...");
        ...
        while (!agenda.isEmpty()) {
            ...
            while (agendaIterator.hasNext()) {
                boolean appliedNonScopingModifier = false;
                History current = agendaIterator.next();
                agendaIterator.remove();

                for (History h : chart) {
                    if (h.category.left != null) {
                        if (h.category.left.toString().equals(current.category.toString()) && !current.mainIndex.equals(h.mainIndex)) {
                            if (nonScopingModifiers.contains(h.mainIndex)) {
                                String combination = h.mainIndex.toString() +"," + current.mainIndex.toString();
                                if (combinations.contains(combination) || !inCombinations(combinations, combination)) {
                                    History combined = combineHistories(h, current);
                                    if (combined != null) {
                                        agendaIterator.add(combined);
                    ...
                    if (current.category.left != null) {
                        if (current.category.left.toString().equals(h.category.toString()) && !current.mainIndex.equals(h.mainIndex)) {
                            String combination = current.mainIndex.toString() +"," + h.mainIndex.toString();
                            if (combinations.contains(combination) || !inCombinations(combinations, combination)) {
                                History combined = combineHistories(current, h);
                                if (combined != null) {
                                    agendaIterator.add(combined);
        ...
        //TODO Testing whether removing intermediate histories works

        /*
        // find the largest set of indices among the histories in the chart
        Integer largestSet = chart.stream().map(x -> x.indexSet.size()).max(Integer::compareTo).get();
        //Remove all histories from chart with smaller size
        chart = chart.stream().filter(x -> x.indexSet.size() == largestSet).collect(Collectors.toList());
         */

        return chart;
    }

    public boolean inCombinations(List<String> combinations, String combination)
    {
        for (String c : combinations)
        {
            List<String> cs = List.of(c.split(","));
            if (cs.containsAll(List.of(combination.split(","))))
            {
                return true;
            }
        }
...
*/
    ...
    public Premise combinePremises(Premise functor, Premise argument, StringBuilder proofBuilder) throws VariableBindingException, ProverException {
        ...
        if (eqs.size() > 0) {
            ...
            if (LLProver1b.checkDuplicateBinding(eqs)) {
