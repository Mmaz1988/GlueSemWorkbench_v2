...
presupDRT(Text,Old,New):-
   setof(Sem,Drs^(t([sem:Drs],Text,[]),resolveDrs(merge(Old,Drs),Sem)),New),
   \+ New=[].

% statt setof/3 im Original finall/3 -> funktioniert aber nicht
...
info:-
   ...
   format('~n> ?- presupDRTTestSuite.     - run the test suite for translating discourses into a DRT  <',[]),
   format('~n> ---------------------------------------------------------------------------------------<',[]),
   format('~n> ?- evaluate(F,E).          - evaluate a drs in a model             			 <',[]),
   format('~n> ?- modelCheckerTestSuite.  - run the test suite for modelChecker   			 <',[]),
   format('~n> ---------------------------------------------------------------------------------------<',[]),
   format('~n~n',[]).
